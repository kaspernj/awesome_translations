...
describe AwesomeTranslations::ModelInspector do
  ...
  it "#engines" do
    expect(AwesomeTranslations::ModelInspector.engines.map(&:class).sort { |class1, class2| class1.name <=> class2.name }).to eq [AwesomeTranslations::Engine, BootstrapBuilders::Engine, Haml::Rails::Engine, Jquery::Rails::Engine, MoneyRails::Engine, Twitter::Bootstrap::Rails::Engine]
  end
